...
describe("check_catalog_overlap() / known_overlap (bloque 3.4)", () => {
  ...
  beforeAll(async () => {
    ...
    const { data: catalogEntry } = await admin.client.from("saas_catalog").select("id").limit(1).single();
    if (!catalogEntry) throw new Error("saas_catalog seed vacío — no se puede probar check_catalog_overlap");
  ...
  it("sin vendor con ese catalog_id: has_overlap false, sin contratos", async () => {
    const { data: catalogEntries } = await admin.client
      ...
      .range(1, 1);
    const unusedCatalogId = catalogEntries?.[0]?.id ?? catalogId;

    const { data, error } = await checkOverlap(admin.client, unusedCatalogId);
  ...
  it("MANAGER_ROLES (org_admin) ve importes; un empleado NO recibe importes en el payload (solo existencia/vendor/depto/owner/nº contratos)", async () => {
    const vendorId = await createVendor(admin, "Overlap Vendor", catalogId);
    await createContract(admin, vendorId, { p_cost_amount: 2400 });

    const managerView = await checkOverlap(admin.client, catalogId);
    ...
    const employeeView = await checkOverlap(employee.client, catalogId);
  ...
  it("un contrato cancelado no cuenta como solapamiento (vendor sigue activo)", async () => {
    const vendorId = await createVendor(admin, "Cancelled Contract Vendor", catalogId);
    ...
    const { data } = await checkOverlap(admin.client, catalogId);
    // Puede haber otros vendors activos del mismo catalog_id de tests
    // anteriores en el mismo describe — se comprueba que ESTE contrato
    // cancelado en concreto no aparece en la lista.
    const matchesCancelled = (data?.contracts ?? []).some((c) => c.vendor_name === "Cancelled Contract Vendor");
    expect(matchesCancelled).toBe(false);
  });

  it("un vendor inactivo no cuenta como solapamiento aunque su contrato siga activo", async () => {
    const vendorId = await createVendor(admin, "Inactive Vendor", catalogId);
    ...
    const { data } = await checkOverlap(admin.client, catalogId);
    const matchesInactive = (data?.contracts ?? []).some((c) => c.vendor_name === "Inactive Vendor");
  ...
  it("no es enumerable contra otra org: el mismo catalog_id con vendor en la org A no aparece para la org B", async () => {
    const vendorId = await createVendor(admin, "Org A Only Vendor", catalogId);
    await createContract(admin, vendorId);

    const { data } = await checkOverlap(otherOrgAdmin.client, catalogId);
    const leaked = (data?.contracts ?? []).some((c) => c.vendor_name === "Org A Only Vendor");
    expect(leaked).toBe(false);
  });

  it("create_purchase_request calcula known_overlap server-side (no es un valor que el cliente pueda afirmar)", async () => {
    const vendorId = await createVendor(admin, "Known Overlap Vendor", catalogId);
    await createContract(admin, vendorId);

    const { data: overlappingRequestId } = await admin.client.rpc(
      "create_purchase_request",
      createRequestParams({ p_catalog_id: catalogId, p_estimated_annual_cost: 8000 }),
    );
