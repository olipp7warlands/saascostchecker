  ...
  return (
    ...
    >
      <div className="mb-5 flex flex-wrap items-baseline justify-between gap-2.5">
        ...
        <span className="num text-[11.5px] text-ink-soft">{t("hint")}</span>
      </div>
          ...
          {TICKETS.map((ticket) => (
            ...
            >
              ...
              <div className={cn("num mt-0.5 text-[10.5px] font-semibold", TONE_TEXT_CLASSES[ticket.tone])}>
                {ticket.tone === "hot"
                  ? t("autoRenewsWarning", { days: ticket.days })
                  : ticket.seats
                    ? t("daysRemainingWithSeats", { days: ticket.days, seats: ticket.seats })
                    : t("daysRemaining", { days: ticket.days })}
              </div>
            </div>
          ))}
        </div>
